...
public class NewSceneHierarchy : EditorWindow
{
    ...
    protected internal override void Draw() {
        ...
        if (ImGui.BeginPopupContextWindow())
        {
            ...
            if (ImGui.Button("Add"))
            {
                ...
                Scene newScene = Osmium.AddScene(GenerateDefaultName())!;
                
                Map.SetVariable(newScene, "Name", name);
        ...
        int count = 0;
        foreach (Scene scene in Map.Scenes.ToArray())
        {
                ...
                if (ImGui.BeginDragDropTarget())
                {
                    ...
                    unsafe
                    {
                        if (payload.NativePtr != null)
                        {
                            ...
                            SceneMapNode grabbedScene = Map.Scenes[grabbingIndex];
                            SceneMapNode targetScene = Map.Scenes[targetIndex];
                                
                            Map.Scenes.RemoveAt(grabbingIndex);
                            
                            int newIndex = Map.Scenes.IndexOf(targetScene);
                            ...
                            if (t <= .5f)
                            {
                                if (newIndex < 0)
                                    newIndex = 0;
                                if (newIndex > Map.Scenes.Count)
                                    newIndex = Map.Scenes.Count;
                                
                                Map.Scenes.Insert(newIndex, grabbedScene);                                
                            }else if (t > .5f)
                            {
    ...
    public void StartRenaming(Scene scene) {
        SelectedScene = scene;
        Renaming = true;
        SetFocus = true;
    ...
    public void FinishRenaming() {
        ...
        if (ValidateName(FinalName))
        {
            ...
            Map.SetVariable(SelectedScene, "Name", FinalName);
        }
    }
